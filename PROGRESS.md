...
| 2026-03-25 | CI green all 4 platforms           | main (direct)             | done   | All 4 CI jobs passing: Windows MSVC x86, Linux GCC cross →Win x86, Linux native GCC x86_64, macOS ARM64 Clang. Added comprehensive compat/ shims (dsound, dinput, oaidl, crtdbg, eh, ole2, dbt, windows.h additions: heap API, GDI stubs, window styles, system metrics, stock objects, LoadIcon/LoadImage, FindWindow, SW_* constants, WS_*/CS_* styles, MAKEINTRESOURCE, SRCCOPY, CallWindowProc). Fixed pointer→UnsignedInt truncation (uintptr_t double-cast). Fixed static/non-static declaration mismatches. Fixed socklen_t on MSVC. Excluded Win32Device from non-Windows builds. |
| 2026-03-25 | P1-11 verified: CD check removal   | main (direct)             | done   | Disabled DO_COPY_PROTECTION in CopyProtection.h. Eliminated 60-second startup hang caused by SafeDisc launcher poll loop (notifyLauncher) on modern Steam/EA App installs. Game now responds within 12 seconds of launch. |
| 2026-03-25 | AsciiString FNV-1a hash fix        | main (direct)             | done   | Fixed rts::hash<AsciiString> in STLTypedefs.h. MSVC 2022 std::hash<const char*> hashes the pointer address, not string content — broke all hash_map (unordered_map) lookups when two AsciiString objects held same content in different buffers. Replaced with FNV-1a byte-level hash. Fixed "Serious Error: ObjectReskin" crash during INI parsing (chemicalgeneral.ini and others). |
| 2026-03-25 | vcpkg Phase 5 deps removed         | main (direct)             | done   | Removed ffmpeg, openal-soft, sdl3 from vcpkg.json (Phase 5 deps). Were causing ~25 min FFmpeg source compile on every CI run. CI build now 3-5 minutes. Only zlib remains (Phase 1 dep). |
| 2026-03-25 | GAME VERIFIED WORKING              | main (direct)             | done   | zh-revival.exe (commit ccafc7f + hash fix + CD fix) launches, plays a skirmish, and exits cleanly on Windows 11 with Steam install of Zero Hour. First verified working build. D3D8 via dxwrapper. |
